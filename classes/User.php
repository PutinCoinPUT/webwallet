...
class User
{
...

	{
		...
		if (($id)) {
			$msg = "<div class='aler alert-success'><center><img src='$qrcode'><br />Secret Key: $secret <br /><h6><strong>*Please write this down and keep in a secure area*<br>Please scan this with the Google Authenticator app on your mobile phone. This page will clear on refresh, please be careful.</stron></h6>";
			$this->mysqli->query("UPDATE users SET authused=1, secret='" . $secret . "' WHERE id=" . $id);
			return "$msg";
		}
	}
	...
	function disauth()
	{
		$id = $_SESSION['user_id'];
		if (($id)) {
			$msg = "<div class='aler alert-success'>Two Factor Auth has been disabled for your account and will no longer be required when you sign in.</div>";
			$this->mysqli->query("UPDATE users SET authused=0, secret='' WHERE id=" . $id);
			return "$msg";
		}
	}
